...
export const VERSION = 'V0.19 / 05.02.2025'
...
export function getRandomString(anz){ // liefert einen RandomString aus Buchstaben und Ziffern
    const cpool="0123456789abcdefghijklmnopqrstuvwxyz"
    let res=""
    while(anz--) res += cpool.charAt(Math.floor(Math.random()*cpool.length))
    return res
}
...
export async function doDialogOK(header, contenthtml, okbuttonhtml = null, confirmflag = false, timeout_sec = 0) {
    try {
        ...
        if (okDialog == undefined) {
            ...
            okDialog.querySelector(".ok-dialog-check input").addEventListener('click', () => {
...
// Content entweder geclonte Nodelist aus Template oder String (HTML)
export function prepareCustomDialog(header, contentho, okhtml = null, buttosextrahtml = null) {
    try {
        joPing()
        ...
        const contelem = customDialog.querySelector(".jo-dialog-content")
        contelem.innerHTML = null
        if(typeof (contentho) === 'string'){
            contelem.innerHTML = contentho
        }else if(typeof contentho === 'object'){
            contelem.appendChild(contentho) 
        }
